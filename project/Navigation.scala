...
object Navigation extends AutoPlugin {
  ...
  private val exerciseParser: Parser[String] =
    Space ~> StringBasic.examples(FixedSetExamples((0 until exerciseList.size).map(_.toString)))
  ...
  private val gotoExerciseHelp = Help(
    "gotoExerciseNr",
    (
      "gotoExerciseNr <exercise number>",
      "Moves to the specified exercise, bringing tests into scope."
    ),
    "Moves to the specified exercise, bringing tests into scope."
  )

  private val gotoExerciseCommand =
    Command("gotoExerciseNr", gotoExerciseHelp)(state => exerciseParser) { case (state, arg) =>
  ...
  private val gotoFirstExerciseCommand =
    Command.command("gotoFirstExercise", gotoFirstExerciseHelp) { state =>
  ...
  private val nextExerciseCommand = Command.command("nextExercise", nextExerciseHelp) { state =>
  ...
  private val prevExerciseCommand = Command.command("prevExercise", prevExerciseHelp) { state =>
  ...
  private val pullSolutionCommand = Command.command("pullSolution", pullSolutionHelp) { state =>
  ...
  private val pullTemplateCommand =
    Command("pullTemplate", pullTemplateHelp)(state => basicParser) { (state, file) =>
      val exercise = currentExercise()
  ...
  private val listExercisesCommand = Command.command("listExercises", listExercisesHelp) { state =>
  ...
  private val showExerciseCommand = Command.command("showExerciseID", showExerciseHelp) { state =>
